...
calcBtn.addEventListener('click', () => {
  ...
  const sName = placeData.start ? placeData.start.display_name : startInput.value.trim();
  const eName = placeData.end ? placeData.end.display_name : endInput.value.trim();
  const sLat = placeData.start ? placeData.start.lat : '';
  const sLon = placeData.start ? placeData.start.lon : '';
  const eLat = placeData.end ? placeData.end.lat : '';
  const eLon = placeData.end ? placeData.end.lon : '';

  if (!sName || !eName) {
    validMsg.textContent = 'Please enter start and end destinations';
    return;
  }
